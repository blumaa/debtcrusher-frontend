...
export const fetchSecondaryBackers = () => {
  ...
  return async dispatch => {
    try {
      const response = await fetch("https://debt-crusher.herokuapp.com/api/secondaryBackers");
...
export const createSecondaryBacker = ({amount, userId, projectId}) => {
  ...
  return async dispatch => {
    try {
      ...
      const response = await fetch('https://debt-crusher.herokuapp.com/api/secondaryBackers', reqObj)
      const json = await response.json()
      // console.log(json)
      ...
      const donObj = {
        ...
        const res2 = await fetch(
          "https://debt-crusher.herokuapp.com/api/users/" + userId + "/donationSubtract",
          donObj
        );
